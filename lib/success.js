...
export default async (pluginConfig, context) => {
  ...
  const { projectApiUrl: parentProjectApiUrl } = getParentContext(gitlabApiUrl);
  const apiOptions = {
...
`;
      ...
      // releaseInfos를 사용하여 Parent에 이슈 생성
      await Promise.all(releaseInfos.map(postIssueToParent));

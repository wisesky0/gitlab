...
import getWaffulParentContext from "./get-wafful-parent-context.js";
...
export default async (pluginConfig, context) => {
  ...
  const { projectApiUrl: waffulParentProjectApiUrl } = getWaffulParentContext(gitlabApiUrl);
  const apiOptions = {
...
Using 'false' for 'successComment' is deprecated and will be removed in a future major version.`);
    ...
    try {
      ...
      // =============== Start of waffulParent에 이슈 생성 ==================
      const postIssueToWaffulParent = async (release) => {
        const issueTitle = `[${context.options.repositoryUrl.split('/').pop()}] ${release.name}`;
       
        // 릴리스 노트가 존재할 경우 그 내용을 사용하고, 그렇지 않을 경우 '변경 사항이 없습니다.'라는 기본 메시지를 사용
        const changelogContent = release.notes || '변경 사항이 없습니다.';
        const issueDescription = `
## 릴리스 정보
${release.name} 버전이 성공적으로 배포되었습니다.
...
`;
        ...
        const issueEndpoint = urlJoin(waffulParentProjectApiUrl, 'issues');
      ...
      // releaseInfos를 사용하여 waffulParent에 이슈 생성
      await Promise.all(releaseInfos.map(postIssueToWaffulParent));
      // =============== End of waffulParent에 이슈 생성 ==================
